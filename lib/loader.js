...
Loader.prototype.loadFixture = function(fixture, models) {
    ...
    return this.prepFixtureData(fixture.data, Model).spread(function(data, many2many) {
        ...
        var findingDuplicate = Promise.resolve(null);
        if (!self.options.skipDuplicateCheck) {
            findingDuplicate = Model.find(findOptions);
        }

        return findingDuplicate.then(function(instance) {
            if (instance) {
                self.skipped++;
...
Loader.prototype.prepFixtureData = function(data, Model) {
    ...
    Object.keys(data).forEach(function(key) {
        ...
        Object.keys(Model.associations).forEach(function(assocName) {
            if(Model.associations[assocName].options && (Model.associations[assocName].options.through == key || Model.associations[assocName].options.as == key)) {
                return foundInThroughName = assocName;
            }
        });
